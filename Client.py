...
class Client:
    ...
    def listen_rtp(self):
        max_rtp_packet = 20480
        rtp_packet = RtpPacket()
        while True:
            try:
                data_raw, address = self.rtpSocket.recvfrom(max_rtp_packet)

                if data_raw:
                    rtp_packet.decode(data_raw)
                    print("||Received Rtp Packet #" + str(rtp_packet.seqnum) + "|| ")
                    try:
                        if self.frameNbr + 1 != rtp_packet.seqnum:
                            self.counter += 1
                            print('!' * 60 + "\nPACKET LOSS\n" + '!' * 60)
                        currFrameNbr = rtp_packet.seqnum
    ...
    def updateMovie(self, imageFile):
        ...
        try:
            photo = PhotoImage(Image.open(imageFile))  # stuck here !!!!!!
        except:
            print("photo error")
            print('-' * 60)
            traceback.print_exc(file=sys.stdout)
